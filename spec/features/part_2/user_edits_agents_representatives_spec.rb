...
describe "User edits submission agent and representative person", js: true do
  ...
  scenario "representative" do
    ...
    within(".modal.fade.in") do
      ...
      select("Corporate", from: "Type")
      click_on("Save Representative Person")
    end

    expect(page).not_to have_link("Add Representative Person")
    expect(page).to have_css(".representative-entity_type", text: "Corporate")
    expect(page).to have_css(".representative-imo_number", text: "1234567")

    click_on("BOB BOLD")
    fill_in("IMO Number", with: "7654321")
    click_on("Save Representative Person")

    expect(page).to have_css(".representative-imo_number", text: "7654321")
